...
public class TraHangGUI extends javax.swing.JPanel {
    ...
    private void btnXacNhanActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btnXacNhanActionPerformed
        ...
        String maHoaDon = txtNhapMaHoaDon.getText().trim();
        int soPTH = traHangBUS.getSoPTH(maHoaDon);

        for (int i = 0; i < dtmCTHD.getRowCount(); i++) {
            if (dtmCTHD.getValueAt(i, 8) == Boolean.TRUE) {
                int sl = Integer.parseInt(dtmCTHD.getValueAt(i, 3).toString());
                if (sl > 0) {
                    if (soPTH > 0) {
...
int chon = JOptionPane.showConfirmDialog(null, "Xác nhận tạo phiếu trả?", "Xác nhận", JOptionPane.YES_NO_OPTION);
        if (chon == JOptionPane.YES_OPTION) {
            PhieuTraHangDTO pth = getPhieuTraHang();
            List<ChiTietPhieuTraHangDTO> list = getListCTPTH(pth);
    ...
    private void addHoaDon(String maHoaDon, HoaDonDTO hoaDon) {
        if (hoaDon == null) return;
        
        int soPTH = traHangBUS.getSoPTH(maHoaDon);
        int tongSoPhieuTraHang = soPTH;
        List<ChiTietHoaDonDTO> listCTHD;
        
        if (soPTH > 0) {
            listCTHD = traHangBUS.getChiTietHoaDonDaTruPTHTheoMaHD(maHoaDon);
        } else {
            listCTHD = traHangBUS.getChiTietHoaDonByMaHoaDon(maHoaDon);
        }

        if (listCTHD == null || listCTHD.isEmpty()) {
            JOptionPane.showMessageDialog(null, "Tất cả sản phẩm trong hóa đơn này đã được trả hết!");
            return;
        }

        long ngayLap = ChronoUnit.DAYS.between(hoaDon.getNgayLapHoaDon(), LocalDateTime.now());
        if (ngayLap <= 30) {
            lblSoLanTra.setText(String.valueOf(tongSoPhieuTraHang));
            lblSoNgayHD.setText(String.valueOf(ngayLap));
            lblTongTienTra.setText(dinhDangTien(traHangBUS.getTongTienCacPTH(maHoaDon)));
            
            themDuLieuCTHDVaoBang(listCTHD);
            taoThongTinPhieu(maHoaDon);
            
            if (hoaDon.getTenKH() != null) {
                txtTenKhachHang.setText(hoaDon.getTenKH());

                if (hoaDon.getMaKH() != null && !hoaDon.getMaKH().equalsIgnoreCase("KH-00000")) {
                    txtDiemTichLuy.setText(String.valueOf(hoaDon.getDiemTichLuyKH()));
                } else {
                    txtDiemTichLuy.setText("0");
                }
            }            
        } else {
            JOptionPane.showMessageDialog(null, "Hóa đơn đã lập " + ngayLap + " ngày (quá 30 ngày), không thể trả hàng!");
        }
    }
    


    private void themDuLieuCTHDVaoBang(List<ChiTietHoaDonDTO> listCTHD) {
        int stt = 0;
        ...
        for (ChiTietHoaDonDTO cthd : listCTHD) {
            ...
            String tenSanPham = cthd.getTenSP();
            int soLuong = cthd.getSoLuong();
            String donViTinh = cthd.getTenDVT();
            ...
            row[0] = stt;
            row[1] = maCTHD;
            row[2] = (soLuong <= 0) ? ("Đã trả hết " + tenSanPham) : tenSanPham;
    ...
    private void themDongBangPhieuTraHang(String maCTHD) {
        ChiTietHoaDonDTO cthd = traHangBUS.getChiTietHoaDonByMa(maCTHD);
        if (cthd == null) return;
        ...
        String giaTriHoanTra = "100%";
        double thanhTienHoanTra = thanhTien;
    ...
    private void themDongBangPhieuTraHangDaTungTraRoi(String maCTHD) {
        ChiTietHoaDonDTO cthd = traHangBUS.getChiTietHoaDonDaTungTraRoiByMa(maCTHD);
        if (cthd == null) return;
    ...
    private List<ChiTietPhieuTraHangDTO> getListCTPTH(PhieuTraHangDTO pth) {
        DefaultTableModel dtm = (DefaultTableModel) tblTraHang.getModel();
        List<ChiTietPhieuTraHangDTO> listPTH = new ArrayList<>();

        if (dtm.getRowCount() == 0) {
            throw new IllegalArgumentException("Không có chi tiết phiếu trả hàng");
        } else {
            for (int i = 0; i < dtm.getRowCount(); i++) {
                String maCTHD = dtm.getValueAt(i, 10).toString();
                
                String truongHopStr = dtm.getValueAt(i, 6).toString();
                String lyDoEnumStr = "HANG_LOI_DO_NHA_SAN_XUAT";
                if (truongHopStr.equals(TruongHopDoiTraEnum.DI_UNG_MAN_CAM.getTruongHopDoiTra())) {
                    lyDoEnumStr = "DI_UNG_MAN_CAM";
                } else if (truongHopStr.equals(TruongHopDoiTraEnum.NHU_CAU_KHACH_HANG.getTruongHopDoiTra())) {
                    lyDoEnumStr = "NHU_CAU_KHACH_HANG";
                }

                String tinhTrangEnumStr = "HANG_NGUYEN_VEN";
                if (dtm.getValueAt(i, 7) == Boolean.FALSE) {
                    tinhTrangEnumStr = "HANG_KHONG_NGUYEN_VEN";
                }
                ...
                ChiTietPhieuTraHangDTO ctpth = ChiTietPhieuTraHangDTO.builder()
                        .maPhieuTraHang(pth.getMaPhieuTraHang())
                        .maChiTietHoaDon(maCTHD)
                        .soLuong(soLuong)
                        .truongHopDoiTra(lyDoEnumStr)
                        .tinhTrangSanPham(tinhTrangEnumStr)
                        .giaTriHoanTra(giaTriHoanTra)
                        .thanhTienHoanTra(thanhTienHoanTra)
                        .build();
                listPTH.add(ctpth);
            }
        }
    ...
    private PhieuTraHangDTO getPhieuTraHang() {
        ...
        return PhieuTraHangDTO.builder()
                ...
                .maNV(maNV)
                .maHoaDon(maHD)
                .ngayLapPhieuTraHang(LocalDateTime.now())
                .tongTienHoaTra(tongTien)
                .build();
    }

    private void taoPhieuTraHang(PhieuTraHangDTO pth, List<ChiTietPhieuTraHangDTO> list) {
        int WIDTH = 171;
        ...
        for (ChiTietPhieuTraHangDTO ct : list) {
            ChiTietHoaDonDTO cthd = traHangBUS.getChiTietHoaDonByMa(ct.getMaChiTietHoaDon());
            String tenSP = (cthd != null) ? cthd.getTenSP() : "N/A";
            if (tenSP.length() > 45) tenSP = tenSP.substring(0, 45);

            String donViTinh = (cthd != null) ? cthd.getTenDVT() : "N/A";
            String tinhTrangSP = "HANG_NGUYEN_VEN".equals(ct.getTinhTrangSanPham())
                    ? "Nguyên vẹn" : "Đã sử dụng";

            String lyDo = ct.getTruongHopDoiTra();
            if ("HANG_LOI_DO_NHA_SAN_XUAT".equals(lyDo)) lyDo = "Hàng lỗi NSX";
            else if ("DI_UNG_MAN_CAM".equals(lyDo)) lyDo = "Dị ứng mẫn cảm";
            else if ("NHU_CAU_KHACH_HANG".equals(lyDo)) lyDo = "Nhu cầu KH";
            ...
            double donGia = (cthd != null) ? cthd.getDonGia() : 0;
            double giamGia = (cthd != null) ? cthd.getGiamGia() : 0;
            double thanhTienGoc = ct.getSoLuong() * donGia - (ct.getSoLuong() * donGia * (giamGia / 100));

            sb.append(String.format(
                "%-4d %-45s %-5d %-10s %-10s %-12s %-15s %-15s %-25s %-15s %-15s\n",
                stt++, tenSP, ct.getSoLuong(), donViTinh,
                df.format(donGia),
                df.format(giamGia * 100) + "%",
